...
export const addDoctorAvailableSlots = async (
  ...
  try {
    ...
    if (startTime < new Date()) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: "Start time must be greater than current time" });
    }

    if (endTime < new Date()) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: "End time must be greater than current time" });
    }
    ...
    if (existingAppointment) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: "This time slot is occupied with an appointment" });
    }

    const existingAvailableSlot = doctor?.availableSlots.find(
    ...
    if (existingAvailableSlot) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: "This time slot is already added" });
    }
