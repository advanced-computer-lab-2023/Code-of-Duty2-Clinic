...
import {
  addDoctorRegistrationFiles,
  createNewDoctorRegistrationRequest,
  getDoctorRegistrationContract,
  rejectSentContract,
...
const registerAsDoctor = async (req: Request, res: Response) => {
  try {
    ...
    res
      ...
      .send("Doctor Registration Request Sent Successfully!");
  } catch (err: any) {
    console.log(err);
...
const addDoctorRegistrationRequestFiles = async (
  ...
  try {
    ...
    res
      .status(StatusCodes.CREATED)
      .send("Doctor Registration Request Files Sent Successfully!");
  } catch (err: any) {
    console.log(err);
    res.status(StatusCodes.BAD_REQUEST).send({ message: err.message });
  }
