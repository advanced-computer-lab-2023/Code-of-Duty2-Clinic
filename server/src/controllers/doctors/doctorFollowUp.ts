...
export const scheduleFollowUp = async (req: AuthorizedRequest, res: Response) => {
  try {
    ...
    if (!doctor || !patient) {
      return res.status(StatusCodes.NOT_FOUND).json({ message: 'Doctor or patient not found' });
    }

    
    const initialAppointment = await findMostRecentCompletedAppointment(doctorId, patient._id);

    if (!initialAppointment) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        message: 'No recent completed appointment found between the doctor and patient'
      });
    }
