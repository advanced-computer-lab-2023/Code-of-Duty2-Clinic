...
export const addPatientRegisteredFamilyMember = async (req: AuthorizedRequest, res: Response) => {
  ...
  try {
    ...
    const requestingPatient = await Patient.findById(patientId).select('+registeredFamilyMembers +registeredFamilyMemberRequests');
  ...
  const familyMember = await Patient.findOne(query);
    ...
    if (familyMember.id === requestingPatient!.id) {
        return res.status(StatusCodes.BAD_REQUEST).json({ message: 'You cannot add yourself as a family member' });
      }

    const registeredFamilyMember: IRegisteredFamilyMember = {
        id: familyMember!.id,
        relation: Relation [relation.toUpperCase() as keyof typeof Relation]
    };


    const requestingFamilyMemberRelation = 
    relation === 'Father' ? (requestingPatient!.gender === 'male' ? 'SON' : 'DAUGHTER') :
    relation === 'Mother' ? (requestingPatient!.gender === 'male' ? 'SON' : 'DAUGHTER') :
    relation === 'Son' ? (familyMember!.gender == 'male' ? 'FATHER' : 'MOTHER') :
    relation === 'Daughter' ? (familyMember!.gender == 'male' ? 'FATHER' : 'MOTHER') :
    relation == 'Husband' ? 'WIFE' :
    relation == 'Wife' ? 'HUSBAND' :
    relation === 'Sibling' ? 'Sibling' :
    relation === 'Other' ? 'Other' : 'Other';
    

    const requestingFamilyMember: IRegisteredFamilyMember = {
        id: requestingPatient!.id,
        relation: Relation [requestingFamilyMemberRelation.toUpperCase() as keyof typeof Relation]
    };
...
if (requestingPatient!.registeredFamilyMemberRequests) {
  requestingPatient!.registeredFamilyMemberRequests = requestingPatient!.registeredFamilyMemberRequests.filter(member => member.id.toString() !== familyMember!.id.toString());
} 
...
if (! (familyMember!.registeredFamilyMemberRequests.some(member => member.id.toString() === requestingPatient!.id.toString()))) {
...
export const deletePatientRegisteredFamilyMember = async (req: AuthorizedRequest, res: Response) => {
...
if (requestingPatient?.registeredFamilyMembers) {
try {
requestingPatient.registeredFamilyMembers = requestingPatient.registeredFamilyMembers.filter(member => member.id.toString() !== familyMemberId);
await requestingPatient.save();
} catch (error) {
console.log(error);
}
...
try {
familyMember.registeredFamilyMemberRequests = familyMember.registeredFamilyMemberRequests.filter(member => member.id.toString() !== patientId);
await familyMember.save();

res.status(StatusCodes.OK).json({ message: 'Family member has been removed' });
} catch (error) {
  res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: 'Internal Server Error' });
  console.log(error);
    }
...
export const rejectPatientRegisteredFamilyMember = async (req: AuthorizedRequest, res: Response) => {
  const familyMemberId = req.params.familyMemberId;
  const patientId = req.user?.id;

  // Find the requesting patient
  const requestingPatient = await Patient.findById(patientId).select('+registeredFamilyMemberRequests');


if (requestingPatient?.registeredFamilyMemberRequests) {
try {
requestingPatient.registeredFamilyMemberRequests = requestingPatient.registeredFamilyMemberRequests.filter(member => member.id.toString() !== familyMemberId);
await requestingPatient.save();
} catch (error) {
console.log(error);
   }
