...
export const findAppointmentDetailsForDoctor = async (doctorId: string, appointmentId: string) => {
  ...
  if (!appointment) throw new Error(entityIdDoesNotExistError("Appointment", appointmentId));
...
export const getDoctorAppointments = async (userId: string, urlQuery: any) => {
  const registeredPatientsAppointments = await getAppointments(false, userId, urlQuery);
  const dependentPatientsAppointments = await getDoctorAppointmentsForDependentPatients(
    userId,
    urlQuery
  );
...
const getDoctorAppointmentsForDependentPatients = async (
  ...
  const existingDependentPatientsAppointments = await DependentFamilyMemberAppointment.find({
    doctorId: doctorId,
  ...
  for (const appointment of existingDependentPatientsAppointments) {
    ...
    if (!patient)
      throw new Error(entityIdDoesNotExistError("Patient", appointment.payerId.toString()));
    ...
    const dependentMember = patient.dependentFamilyMembers.find(
      (member) => member.nationalId === appointment.dependentNationalId.toString()
    )!;
    ...
    if (
      query.targetName &&
      !dependentMember.name.toLowerCase().startsWith(query.targetName.toLowerCase())
    ) {
      continue;
    }
    result.push({
...
export const rescheduleAppointmentAsDoctorForDependentPatientAndNotifyUsers = async (
  appointmentId: string,
  timePeriod: TimePeriod
) => {
...
export const cancelAppointmentAsDoctorForRegisteredPatientAndNotifyUsers = async (
  appointmentId: string
) => {
  await cancelAppointmentForRegisteredPatientAndNotifyUsers(appointmentId, UserRole.DOCTOR);
};

export const cancelAppointmentAsDoctorForDependentPatientAndNotifyUsers = async (
  appointmentId: string
) => {
  await cancelAppointmentForDependentAndNotifyUsers(appointmentId, UserRole.DOCTOR);
};
