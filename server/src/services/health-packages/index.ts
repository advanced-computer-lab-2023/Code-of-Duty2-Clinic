...
export const findAllHealthPackagesAfterDiscount = async (patientId: string) => {
   ...
   const registeredFamilyMembersDiscounts = await getRegisteredFamilyMembersDiscounts(patient);

   const maxHealthPackagesDiscountPossible = getMaxDiscount(registeredFamilyMembersDiscounts);
...
const getHealthPackagesDetailsAfterDiscount = (
   ...
   const healthPackagesDetailsAfterDiscount = healthPackages.map(healthPackage => {
      ...
      if (healthPackagePriceAfterDiscount !== healthPackage.amountToPay)
         healthPackageDetailsAfterDiscount.amountBeforeDiscount = healthPackage.amountToPay;
...
const getRegisteredFamilyMembersDiscounts = async (patient: IPatientModel) => {
   const registerdFamilyMemberDiscountsPromise = patient.registeredFamilyMembers!.map(
      async member => {
