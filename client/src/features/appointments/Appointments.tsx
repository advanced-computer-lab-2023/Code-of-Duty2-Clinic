...
const Appointments: React.FC<Props> = ({ isPatient }) => {
  ...
  const handleChangeTargetName = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const fetchFilteredAppointments = async() => {
    try {
      ...
      const response = await axios.get(`${config.serverUri}/${isPatient ? 'patients': 'doctors'}/appointments`, { params });
