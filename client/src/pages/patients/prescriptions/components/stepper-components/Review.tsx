...
const Review: React.FC = () => {
  const { order, updateOrder, setStep, prescription } = useContext(PrescriptionCheckoutContext);
  console.log(order);
  useEffect(() => {}, [order]);
  const handleSubmit = () => {
    ...
    <Fragment>
      ...
      <List>
              ...
              >
                <CardContent sx={{ flex: "1 0 auto", m: 0, py: 0 }}>
                  ...
                  <Box sx={{ display: "flex" ,justifyContent:'space-between'}}>
                    <Box>
                      {medicine.priceBefore !== medicine.priceAfter && (
                        <Typography
                    ...
                    <Typography
                      sx={{mt:'auto', display: "inline", color: "green", fontWeight: 600 }}
                      variant="subtitle1"
                      color="text.secondary"
                      component="div"
                    >
                      EGP {medicine.priceAfter! * medicine.quantity}{" "}
                    </Typography>
                  </Box>
                </CardContent>
              </Box>
        ...
        <ListItem key={"smdfkm"} sx={{ py: 1, px: 0 }}>
          <Typography
            variant="subtitle1"
            sx={{ fontSize: "20px", color: "gray", fontWeight: 600, marginRight: 1 }}
          >
            TOTAL:
          </Typography>
          <Typography
            variant="subtitle1"
            sx={{ fontSize: "20px", color: "green", fontWeight: 600, marginRight: 1 }}
          >
            {prescription?.medicines &&
