...
const Prescriptions: React.FC = () => {
  ...
  const supervisingPatientId = useQueryParams().get("spId") as string;
  ...
  const {
    ...
    "infinite",
    ({ pageParam = "3/2/3500" }) =>
      fetchPrescriptions({ pageParam, patientId: patientId!, supervisingPatientId }),
    {
  ...
  async function sumbitPrescriptionToPharamcy(prescriptionId: string) {
    const mutationInput = {
      prescriptionId,
      patientId: patientId!,
      dependent: !!supervisingPatientId
    };
    submitPrescriptionMuatation.mutate(mutationInput);
    await queryClient.invalidateQueries("infinite");
    refetch();
  }

  async function handleCreatePrescription() {
    setAdding(true);
    createPrescriptionMutation.mutate({ patientId: patientId!, supervisingPatientId });
    await queryClient.invalidateQueries("infinite");
    await refetch();
    setAdding(false);
  }
    ...
    <div>
      ...
      {isError && (
        <Alert
          ...
          sx={{ mt: 4, mb: 2 }}
          action={
            <IconButton aria-label="close" color="inherit" size="small" onClick={() => refetch()}>
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      ...
                      <Typography sx={{ marginLeft: "auto", paddingX: "20px", color: "white" }}>
                        {prescription.isSubmitted ? (
                          <Chip
                            label="Submitted"
                            sx={{
                              backgroundImage: "linear-gradient(to right, #178c36, #00ff99)",
                              fontWeight: 500,
                              color: "white",
                              width: 180,
                              opacity: 0.5
                    ...
                    <AccordionDetails>
                      {!prescription.isSubmitted ? (
                        ...
                        <DownloadablePDF prescription={prescription} patientId={patientId} supervisingPatientId={supervisingPatientId} />
                      )}
